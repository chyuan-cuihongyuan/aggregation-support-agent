...
public class InternalDocsTools {
     ...
     */
    @Tool(description = "查询内部知识库文档，通过语义检索获取与查询最相关的资料，包括但不限于：产品信息与价格、运维文档、故障案例、架构说明、技术规范、业务数据等。当用户询问任何可能存在于知识库中的具体信息时，都应使用此工具。【构造查询的关键规则】必须原样保留用户提及的所有具体型号、商品名、品牌、编号等实体词（如 \"i5-14700KF\"、\"RTX 4070 Ti\"、\"SN850X 2TB\"），严禁用 \"电脑硬件\"、\"配件\"、\"商品\" 等笼统类别词概括——概括会直接导致检索不到对应的价格或规格；宁可把多个实体词拼进同一条查询，也绝不可丢弃。若一次需要查询多个独立条目，请改用 queryInternalDocsBatch 一次性批量查询，避免逐条串行检索拖慢响应")
     ...
     */
    @Tool(description = "批量查询内部知识库文档 — 一次性并行检索多个查询主题。当用户的问题涉及多个独立条目（如同时询问多个商品/型号的价格、多个故障案例、多份文档）时，必须使用此工具，为每个具体条目生成一条独立查询（例如用户问多个型号的进货价，应传入 [\"i5-14700KF 进货价\", \"RTX 4070 Ti 进货价\", \"铠侠 SN850X 2TB 进货价\"]），一次性传入全部查询，而不是多次调用 queryInternalDocs，以并行检索大幅缩短响应时间。每条查询都必须保留具体型号/商品名/品牌等实体词，严禁概括成 \"电脑硬件\"、\"配件\" 等类别词")
