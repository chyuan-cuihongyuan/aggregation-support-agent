-- 迁移脚本: 新增知识库分类表，并给文档元数据增加所属知识库字段
...
ALTER TABLE `document_metadata`
  ADD COLUMN `knowledge_base_id` varchar(64) NOT NULL DEFAULT '' COMMENT '所属知识库ID' AFTER `owner_user_id`;

ALTER TABLE `document_metadata`
  ADD COLUMN `knowledge_base_name` varchar(128) NOT NULL DEFAULT '' COMMENT '所属知识库名称' AFTER `knowledge_base_id`;

ALTER TABLE `document_metadata`
  ADD INDEX `idx_kb_scope` (`tenant_id`, `owner_user_id`, `knowledge_base_id`, `deleted_flag`);
